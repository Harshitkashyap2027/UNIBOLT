...
window.validateStep1 = function() {
    if (window.checkEmpty(['inp-name', 'inp-phone', 'inp-city', 'inp-state', 'inp-college', 'inp-course'])) {
        ...
        window.showWarning("Please fill in all required details to proceed.");
    }
};
...
window.validateStep2 = function() {
    ...
    if (!utr.value.trim() || utr.value.length < 12) {
        ...
        window.showWarning("Please enter a valid 12-digit UTR / Transaction ID.");
        return;
    }
...
window.handleAppPayment = function() {
    ...
    if (!upi) { window.showWarning("Enter your UPI ID first."); return; }
...
// --- FIREBASE REGISTRATION (APP DEVELOPMENT SPECIFIC) ---
window.registerUser = async function() {
    ...
    if (p1.length < 8) {
        ...
        window.showWarning("Password must be at least 8 characters.");
        return;
    }
    ...
    try {
        ...
        await setDoc(doc(db, "users", user.uid), {
            ...
            enrolledProgram: "App Development",
        ...
        window.showWarning("Registration failed: " + msg);
