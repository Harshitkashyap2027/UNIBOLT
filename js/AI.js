...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        // B. MAINTENANCE MODE CHECK (New Feature)
        try {
            const settingsSnap = await getDoc(doc(db, "settings", "global"));
            if (settingsSnap.exists() && settingsSnap.data().maintenance === true) {
        ...
        // --- D. NEW: LOAD AI MISSIONS (Added Feature) ---
        loadLatestTask();
        loadMissionLog();
...
// --- 5. LOCKDOWN MODE (Restored Full Logic) ---
function activateLockdown(userData) {
    ...
    links.forEach(link => {
        ...
        // Allow: /AI (Dashboard), /aicontact (Support), Logout
        if (href !== '/AI' && href !== '/aicontact' && !link.classList.contains('logout-btn')) {
            link.style.opacity = "0.5";
            link.style.cursor = "not-allowed";
            // Add lock icon if not already there
            if(!link.innerHTML.includes('fa-lock')) {
                link.innerHTML += ' <i class="fa-solid fa-lock" style="font-size:10px; margin-left:5px;"></i>';
            }
            // Prevent click
            link.onclick = (e) => { 
                e.preventDefault(); 
    ...
    // 3. Disable Action Buttons (Restored Feature)
...
// --- 7. UI UPDATER (Restored Avatar Logic) ---
function updateDashboardUI(data) {
    ...
    // Avatar Logic (Restored Specific Styling)
    if (dom.avatar) {
        if (data.avatarIcon) {
            dom.avatar.innerText = data.avatarIcon; // Use Emoji
            dom.avatar.style.fontSize = "20px";
            dom.avatar.style.background = "rgba(255,255,255,0.1)"; 
        } else {
            dom.avatar.innerText = userName.charAt(0).toUpperCase(); // Use Initial
    ...
    const progressPct = Math.min(100, Math.round((completedCount / TOTAL_AI_MODULES) * 100));
...
function renderNotifications(notifs) {
    ...
    if (dom.notifList) {
        ...
        [...notifs].reverse().forEach(n => {
            ...
            // Restored original notification HTML structure
            item.innerHTML = `
                <div style="color: #8b5cf6; margin-top: 2px;">
...
// Global Announcements (Admin Broadcasts)
...
// --- 11. COMMUNITY FEED (Restored Colors) ---
const aiEvents = [
    { user: "Aarav S.", action: "trained CNN", target: "98% Acc", time: "2m ago", icon: "fa-brain", color: "#8b5cf6" },
    { user: "Priya M.", action: "unlocked", target: "Kaggle Dataset", time: "5m ago", icon: "fa-database", color: "#f59e0b" },
    { user: "Rahul K.", action: "deployed", target: "Chatbot", time: "12m ago", icon: "fa-robot", color: "#10b981" },
    { user: "Sneha L.", action: "visualized", target: "Big Data", time: "25m ago", icon: "fa-chart-pie", color: "#3b82f6" }
];
...
// --- 13. AI MISSION CONTROL (NEW ADDED FEATURES) ---

// A. Load "Latest Task" Widget (Populates the top alert)
async function loadLatestTask() {
    const widget = document.getElementById("latest-task-widget");
    if(!widget) return;

    try {
        // Find most recent task for AI
        const q = query(
            collection(db, "tasks"), 
            where("module", "==", "AI & Data Science"),
            orderBy("day", "desc"), 
            limit(1)
        );
...
// B. Load "Mission Log" (Populates the list of all AI tasks)
async function loadMissionLog() {
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            div.innerHTML = `
                <div style="width: 40px; height: 40px; background: rgba(139, 92, 246, 0.1); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #8b5cf6; font-weight: bold; font-size: 14px;">
                    ${t.day}
                </div>
                <div style="flex: 1;">
...
function updateDailyTaskDisplay() {
    ...
    const startDate = new Date("2025-12-01"); // YYYY-MM-DD
